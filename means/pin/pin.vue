<template>
  <view class="pin">
    ...
    <view v-if="tabCurrentIndex == 0 ">
      <None @emits="jump_all_pro"
        v-if="navList[0].orderList=='' && list_empty"
        guang="去逛逛"></None>
      <view class='product'
        v-for="(item,index) of navList[0].orderList"
        :key="index"
        v-else>
        <view class='pro_l'>
          <view class='pro_l_pic'
            @click="jump_pro(item.goods_id)">
          ...
          <view class='pro_r_02'>
            <view class='pro_r_02_l'>
              <view class='pro_r_02_l_02'>
                拼购价: <span>¥{{(item.goods.price*10000 - item.price*10000)/10000}}</span>
                <!--  -->
              </view>
              <view>单买价：{{item.goods.market_price}}</view>
            </view>
            <view class='pro_r_02_r'
              @click="jump_pro(item.goods_id)">
              <view class="pro_qcj">去拼购</view>
            </view>
          </view>
        </view>
      </view>
    ...
    <view v-if="tabCurrentIndex == 1">
      <None @emits="jump_all_pro"
        v-if="navList[1].orderList=='' && list_empty"
        guang="去逛逛"></None>
      <view class='product'
        v-for="(item,index) of navList[1].orderList"
        :key="index"
        v-else>
        <view class='pro_l'>
          <view class='pro_l_pic'
