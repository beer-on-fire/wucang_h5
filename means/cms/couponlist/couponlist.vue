<template>
  <view class="coupon">
    <view class='po'>
      <!-- <view class='tab'>
				<view :class="c_index==0?'xz':'bb' " @click="num(0)">店铺红包</view>
            ...
            <view class='cou_t'>
              ...
              <view class='cou_t_r'>
                ...
                <view class='cou_t_r_02'
                  v-if="item.day">有效期：{{item.day}}天</view>
                <view class='cou_t_r_02'
                  v-if="item.start_time||item.end_time">有效期：{{item.start_time}}至{{item.end_time}}</view>
              </view>
              <view class="cou_t_rr"
                v-if="item.is_show == 0"
                @click="del_coupon(item.id)">删除</view>
            </view>
            ...
            <view class='cou_t'>
              ...
              <view class='cou_t_r'>
                ...
                <view class='cou_t_r_02'>
                  <view class='cou_t_r_02'
                    v-if="item.day"
                    style="display: flex;justify-content: space-between;width: 80%;">有效期：{{item.day}}天
              ...
              <view v-if="state==1">
                <view class="btn_grey"
                  v-if="item.is_show == 1"
                  @click="del_coupon(item.id)">撤销</view>
                <view class="no-btn"
                  v-if="item.is_show == 2">撤销中...</view>
              </view>
            </view>
            <view class='cou_d'>描述信息</view>
          </view>
      ...
      <view class="coupon_btn">
        <navigator url="/pages/cms/coupon/coupon">
          <view class=" flex flex-direction">
            <button class="cu-btn bg-red margin-tb-sm lg">添加店铺红包</button>
          </view>
        </navigator>
      </view>
    </view>
