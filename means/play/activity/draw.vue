<template>
	<view class="conbox draw">
							...
							<view class="canvas-item" :style="{ transform: 'rotate(' + index * width + 'deg)', zIndex: index }" v-for="(iteml, index) in list"
							 :key="index">
								<view class="canvas-item-text" :style="'transform:rotate(' + index + ')'">
									...
									<img :src="web_url+ iteml.img" class="jp-img" />
								</view>
							</view>
						</view>
					</view>
		...
		<view v-if="is_mask" class="menban"></view>
		<Masks v-if="is_mask" :gift_list="gift_list" :zj_list="zj_list" :content="content" @close="close"></Masks>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			get_dzp_content(){
				let arr = []
				this.$api.http.get('games/get_dzp_rule?id='+this.gid).then(res=>{
				...
				this.$api.http.get('games/start?id='+this.gid).then(res=>{
					if(res.err_code != 9999){
						for (let k in this.list) {
							let v = this.list[k]
							v['isNoPrize'] = true
							if(v.value == res.data.value){
								index = k
								v['isNoPrize'] = false
							}
						}
						console.log('中奖下标',index)
						console.log(this.list)
						this.animation(index, duration);
						
						setTimeout(() => {
							uni.showModal({
								content: this.list[index].isNoPrize ? '抱歉，您未中奖' : '恭喜，获得'+this.list[index].name,
								success: res => {
									that.get_my_gift()
									that.get_zjzj()
								}
