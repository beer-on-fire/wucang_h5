<template>
	<view class="page">
				...
				<view class="user">
					...
					<view class="info">
						...
						<view class="drop" hover-class="none">
							...
							<view class="icon">
								<text  class="plus" :class="{'pluss': pluss}">+{{config.tree_experience}}</text>
							</view>
						</view>
					</view>
				</view>
		...
		<view class="explain">
			<view>成长进度说明：</view>
			<view class="text">1、每浇水1次，经验+{{config.tree_experience}}。</view>
			<view class="text">2、小树(经验值0-{{config.tree_middle}})， 中树(经验值{{config.tree_middle}}-{{config.tree_big}})， 大树(经验值{{config.tree_big}}以上)。</view>
			<view class="text">3、小树每次浇水获取{{config.tree_small_point}}积分，中树每次浇水获取{{config.tree_middle_point}}积分，大树每次浇水获取{{config.tree_big_point}}积分。</view>
		</view>
...
<script>
	import Cache from '../../../common/cache/user.js'
	import {
		Api_url
	} from '@/common/config'
	export default {
		...
		methods: {
			get_user_info() {
				this.config = uni.getStorageSync('switch')
				...
				this.$api.http.get('user/info').then(res => {
					...
					this.info.votes = my.data.tree_experience
					console.log(this.info)
					let tree_img = my.data.tree_experience
					console.log('树经验',tree_img)
					let mupiao = 0
					if (tree_img < this.config.tree_middle*1) {
						
						mupiao = 1
					}
					if (tree_img > this.config.tree_middle*1 && tree_img < this.config.tree_big*1) {
						mupiao = 2
					}
					if (tree_img > this.config.tree_big*1) {
						mupiao = 3
					}
					console.log("树大",mupiao)
					this.mupiao = mupiao
				})
			...
			water() {
				if (this.wateroff) {
					...
					// 	return
					// }
					this.$api.http.get('games/tree').then(res => {
						if (res.err_code != 9999) {
							this.watercss = true;
							this.wateroff = false;
							...
							setTimeout(() => {
								this.info.votes += that.config.tree_experience*1;
								this.pluss = true;
								this.treemove = true;
								this.movetree = false;
							}, 2000);
							...
							setTimeout(() => {
								...
								this.watercss = false;
								this.treemove = false;
								if (that.config.tree_middle <= this.info.votes && that.config.tree_big > this.info.votes) {
									this.mupiao = 2;
								};
								if (that.config.tree_big <= this.info.votes) {
									this.mupiao = 3;
								};
							}, 4000);
							this.$api.http.get('/user/info').then(res => {
