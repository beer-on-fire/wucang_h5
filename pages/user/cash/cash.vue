<template>
  <view class="cash">
    ...
    <view class="H10"></view>
    <view class='money'>
      <view class='ktx'>可提现金额：{{can_money}}元</view>
    </view>
    <view class='jine'>
...
export default {
  ...
  methods: {
    ...
    async cash () // 提现接口，直接使用
    {
      ...
      if (this.money * 1 > this.can_money * 1) {
        uni.showToast({
          icon: "none",
          title: "最多提现" + this.can_money + "元",
          duration: 2000
        })
        return
      }
      ...
      console.log(obj)
      if (obj.status !== 200) {
        this.$api.msg('出现异常，请稍后重试！')
      } else {
        this.$api.msg('已发起提现请求，请耐心等待！')
      }
      setTimeout(() => {
        uni.navigateBack({})
      }, 1000)
    ...
    async getData () {
      ...
      const res = await this.$api.http.post('fx/user/apply_qiye', params)
      return res
    },
    //公众号支付
    // wxPay (json) {
