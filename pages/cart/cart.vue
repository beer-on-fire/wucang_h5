<template>
  <view class="cart">
    ...
    <view class="cart-list">
      <block v-for="(item, k) in cartList"
        :key="item.id">
        <view class="cart-item">
          <view class="image-wrappers">
            <img :src="getimg+item.imgs"
              @click="jump_detail(item.goods_id)"></img>
            <view class="yticon icon-xuanzhong2 checkbox"
              :class="{checked: item.radio}"
              @click="_radio('item', k)"></view>
          </view>
          <view class="item-right">
            <text class="clamp title"
              @click="jump_detail(item.goods_id)">{{item.goods_name}}</text>
            <text class="attr">{{item.attr_val}}</text>
            <text class="price">¥{{item.price}}</text>
            <view class="item-right-num">
              <tui-numberbox :max="Number(item.stock)"
                :min="1"
                :value="item.num"
                @change="numberChange($event,k)"></tui-numberbox>
            </view>
          </view>
          <text class="del-btn yticon icon-fork"
            @click="deleteCartItem(k)"></text>
        </view>
      </block>
    </view>
    <!-- 底部菜单栏 -->
