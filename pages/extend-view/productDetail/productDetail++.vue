  ...
  <view class="container">
    ...
    <view class="tui-header-box"
      ...
      <view class="tui-header-icon"
        ...
        <view class="tui-icon tui-icon-arrowleft tui-icon-back"
          :style="{color:opcity>=1?'#000':'#fff',background:'rgba(0, 0, 0,'+iconOpcity+')'}"
          @tap="back"></view>
    ...
    <view class="tui-banner-swiper">
      <swiper :autoplay="is_auto"
        ...
        <swiper-item v-if="list.video">
          <video :style="{height:scrollH+'px'}"
            style="width: 100%;"
            id="myVideo"
            :src="getimg+list.video.url"
      ...
      <tui-tag type="translucent"
        shape="circleLeft"
        size="small">{{bannerIndex+1}}
        /{{banner_length}}</tui-tag>

      <view class="xszk"
        v-if="label_name && zk_status=='start'">{{label_name}}</view>
    </view>
    <!--banner-->
    <!-- #ifdef MP-WEIXIN -->
    <button class="btn1"
    ...
    <view class="tui-pro-detail">
        ...
        <block v-if="zk_status=='start'">
          <view class="detail">
            ...
            <view class="time">
              ...
              <uni-countdown :day="discount_time.days"
                border-color="#FF4342"
                color="#FF4342"
                splitor-color="#fff"
                :hour="discount_time.hours"
                :minute="discount_time.minutes"
                :second="discount_time.seconds"
                @timeup="timeup(1)">
              </uni-countdown>
        ...
        <view class="tui-pro-titbox">
          ...
          <button @click="is_share=true"
            ...
            <tui-tag type="gray"
              ...
              shape="circleLeft"
              size="small">
              <view class="tui-icon tui-icon-partake"
      ...
      <view class="tui-discount-box tui-radius-all tui-mtop"
        ...
        <view class="tui-list-cell"
          ...
          <view class="tui-tag-coupon-box">
            <template v-for="(item,index) of couponList"
              v-if="index <= 2">
              <tui-tag size="small"
                type="red"
                shape="circle"
                tui-tag-class="tui-tag-coupon">满{{item.full}}减{{item.reduce}}</tui-tag>
            </template>
          </view>
          <tui-icon name="more-fill"
            :size="20"
            class="tui-right tui-top40"
            color="#666"></tui-icon>
        </view>
      </view>
              ...
              <view class="pt_m">
                ...
                <view class="pt_m_2">剩余
                  <uni-countdown :show-day="false"
                    ...
                    color="#6D6D6F"
                    :hour="remain[index].h"
                    :minute="remain[index].m"
                    :second="remain[index].s"
                    @timeup="timeup(2)"></uni-countdown>
                </view>
              </view>
              <view class="pt_r"
      ...
      <view class="haoyou tui-radius-all tui-mtop"
        v-if="is_pt && list.pt.pt.is_cou_tuan == 1">
      ...
      <view class="tui-cmt-box tui-mtop tui-radius-all">
        ...
        <view class="tui-cmt-content tui-padding"
          ...
          <view class="tui-cmt-user"
            v-if="applist.user">
            <image :src="applist.user.headpic"
              class="tui-acatar"></image>
      ...
      <view class="pro-content">
        <view class="tui-nomore-box">
          <tui-nomore text="详情介绍"
            :visible="true"
            bgcolor="#f7f7f7"></tui-nomore>
        </view>
    ...
    <view class="tui-operation">
      <view class="tui-operation-left tui-col-5 ">
        <view class="tui-operation-item pad"
          hover-class="opcity"
          :hover-stay-time="150"
          @click="jump_tohome">
          <tui-icon name="shop"
        ...
        <view class="tui-operation-item "
          ...
          <tui-badge type="danger"
            size="small"
            v-if="shop_car_num>0">{{shop_car_num}}</tui-badge>
        </view>
        <view class="tui-operation-item pad"
          hover-class="opcity"
          :hover-stay-time="150"
          @click="collecting">
          <tui-icon :name="collected?'like-fill':'like'"
            :size="22"
            :color=" collected?'#ff201f':'#333' "></tui-icon>
          <view class="tui-operation-text tui-scale-small"
            :style="collected?'color: #ff201f;':''">收藏</view>
        </view>
        <view style="width: 20px;"></view>
      </view>
      <view class="tui-operation-right tui-right-flex tui-col-7 tui-btnbox-4">
        <view class="tui-flex-1"
          v-if="list.pt && !list.pt.price && cart_switch ">
          <tui-button type="danger"
            shape="circle"
            size="mini"
            @click="showPopup('car')">加入购物车</tui-button>
        </view>
        <view class="tui-flex-1"
          v-if="list.pt && list.pt.price">
          <tui-button type="warning"
            shape="circle"
            size="mini"
            @click="showPopupxx">我要开团</tui-button>
        </view>
        <view class="tui-flex-1"
          v-else>
          <tui-button type="warning"
            shape="circle"
            size="mini"
            @click="showPopup('shopping')">立即购买</tui-button>
        </view>
      </view>
    </view>
    <!--底部操作栏--->
    ...
    <tui-bottom-popup :show="popupShow"
      ...
      <view class="tui-popup-box">
        ...
        <scroll-view scroll-y
          class="tui-popup-scroll">
          <view class="tui-scrollview-box">
            <template v-for="(item,index) of sku_arr.tree">
              <view class="tui-bold tui-attr-title">{{item.k}}</view>
              <view class="tui-attr-box">
                <view :class="list.sku_arr.initialSku['s'+(index+1)]==i.id?'tui-attr-item-active':'tui-attr-item'"
                  v-for="(i,j) of item.v"
            ...
            <view class="tui-number-box tui-bold tui-attr-title">
              ...
              <tui-numberbox :max="list.stock*1"
                :min="1"
                :value="num"
                @change="sku_change_num"></tui-numberbox>
            </view>
        ...
        <view class="tui-icon tui-icon-close-fill tui-icon-close"
      ...
      <view class="mask-content">
        ...
        <view class="coupon-item"
          ...
          <view class="dott">
            ...
            <div class="an"
              @click="lq_coupon(item.id)">立即领取</div>
          </view>
    ...
    <view class="sha_tan"
      ...
      <view class="share_tan">
          ...
          <view class='s_t_x'>
            <view class='s_t_l'>
              <button open-type="share"
                class="s_t_l_s share"><img src='@/imgs/share1.png' /></button>
            </view>
            <view class='s_t_l'>
              <view class='s_t_l_s'
                @click="shareFc"><img src='@/imgs/share2.png' /></view>
            </view>
          </view>
          <view class='s_t_x'>
            <view class='s_t_l'>
              <button open-type="share"
                class="share">分享好友</button>
            </view>
            <view class='s_t_l'>
        ...
        <view class="bye"
          @click="is_share=false">取消</view>
      </view>
    </view>
    <hchPoster ref="hchPoster"
      :canvasFlag.sync="canvasFlag"
      @cancel="canvasCancel"
      :posterObj.sync="posterData" />
    ...
    <view class="tan_pindan"
      v-if="is_pindan">
      <view class="mask"
        @click="is_pindan=false"></view>
      <view class="tan_pd">
        <view class="close"
          @click="is_pindan=false">
          <tui-icon name="close"
            :size="30"
            color='#000'></tui-icon>
        </view>
...
export default {
  ...
  methods: {
    ...
    _load () {
      ...
      // 	id: this.id
      // }).then(res => {
      productModel.getProduct(this.id).then(res => {
        ...
        if (res.data.pt.price) {
          ...
          // 	id: this.id
          // }).then(res => {
          productModel.getPtItem(this.id).then(res => {
    ...
    is_like (id) {
      ...
      // 	id: id
      // }).then(res => {
      productModel.postIsLike(this.id).then(res => {
        if (!res.data) {
