  ...
  <view class="container">
    ...
    <view class="tui-header-screen"
      ...
      <view class="tui-screen-top">
        <view class="tui-top-item tui-icon-ml"
          :class="[tabIndex==0?'tui-active tui-bold':'']"
          data-index="0"
          @tap="screen">
          <view>{{selectedName}}</view>
          <tui-icon :name="selectH>0?'arrowup':'arrowdown'"
            :size="14"
            :color="tabIndex==0?'#e41f19':'#444'"
            tui-icon-class="tui-ml"></tui-icon>
        </view>
        <view class="tui-top-item"
          :class="[tabIndex==1?'tui-active tui-bold':'']"
          @tap="screen"
          data-index="1">销量</view>
        <view class="tui-top-item"
          @tap="screen"
          data-index="2">
          <tui-icon :name="isList?'manage':'listview'"
            :size="isList?22:18"
            :bold="isList?false:true"
            color="#333"></tui-icon>
        </view>
        <view v-if="!is_fx"
        ...
        <!--下拉选择列表--综合-->
        <view class="tui-dropdownlist"
          :class="[selectH>0?'tui-dropdownlist-show':'']"
        ...
        <view class="tui-dropdownlist-mask"
          :class="[selectH>0?'tui-mask-show':'']"
      ...
      <view class="tui-product-container">
        ...
        <block v-for="(item,index) in productList"
          ...
          <view class="tui-pro-item"
            ...
            <image v-if="item.imgs"
              :src="getimg+item.imgs"
              class="tui-pro-img"
            ...
            <view class="tui-pro-content">
              ...
              <view>
                <view class="tui-pro-price">
                  ...
                  <xianshi v-if="!is_fx && item.discount && item.discount.reduce_price"
                    title="限时"
                    :price="item.price-item.discount.reduce_price*1"></xianshi>
                  <xianshi v-if="!is_fx && item.pt && item.pt.price"
                    title="拼购"
                    :price="item.price-item.pt.price*1"></xianshi>
                  <xianshi v-if="is_fx"
                    title="佣金"
                    :price="item.fx_goods.price*1"></xianshi>
                </view>
    ...
    <!--顶部下拉筛选弹层 属性-->
    <tui-top-dropdown bgcolor="#f7f7f7"
      :show="dropScreenShow"
      :paddingbtm="110"
      :translatey="dropScreenH"
      @close="btnCloseDrop">
      <scroll-view class="tui-scroll-box"
        scroll-y
        :scroll-top="scrollTop">
        <view class="tui-seizeaseat-20"></view>
        <view class="tui-drop-item tui-icon-middle"
          :class="item.selected?'tui-bold':''"
      ...
      <view class="tui-drop-btnbox">
        <view class="tui-drop-btn tui-btn-white"
          hover-class="tui-white-hover"
          :hover-stay-time="150"
          @tap="reset">重置</view>
        <view class="tui-drop-btn tui-btn-red"
          hover-class="tui-red-hover"
          :hover-stay-time="150"
          @tap="btnSure">确定</view>
      </view>
    </tui-top-dropdown>
    ...
    <!--左抽屉弹层 筛选 -->
    <tui-drawer mode="right"
      :visible="drawer"
      @close="closeDrawer">
      <view class="tui-drawer-box"
        :style="{paddingTop:height+'px'}">
        <scroll-view class="tui-drawer-scroll"
          scroll-y
          ...
          <view class="tui-drawer-content tui-flex-attr">
            <view class="tui-attr-item"
              :class="currentId == 0? 'tui-btmItem-active': ''"
              @click="set_all">
              <view class="tui-attr-ellipsis">全部</view>
            </view>
            <block v-for="(item,index) of flist"
              :key="index">
              <view class="tui-attr-item"
                :class="currentId == item.category_id? 'tui-btmItem-active': ''"
                @click="tabtap(item)">
          ...
          <block v-if="currentId>0">
            ...
            <view class="tui-drawer-content tui-flex-attr">
              <block v-for="(sitem,index) of slist"
                :key="index"
                v-if="sitem.pid==currentId">
                <view class="tui-attr-item"
                  :class="sitem.category_id == current_children_id? 'tui-btmItem-active': ''"
                  @click="tab_children(sitem.category_id)">
        ...
        <view class="tui-attr-btnbox">
          <view class="tui-attr-safearea">
            <view class="tui-drawer-btn tui-drawerbtn-black"
              hover-class="tui-white-hover"
              :hover-stay-time="150"
              @click="reset_choose">重置</view>
            <view class="tui-drawer-btn tui-drawerbtn-primary"
              hover-class="tui-red-hover"
              :hover-stay-time="150"
              @tap="closeDrawer">确定</view>
          </view>
        </view>
      </view>
    </tui-drawer>
    ...
    <tui-nomore :visible="!pullUpOn && isList"
  ...
  methods: {
      ...
      if (index == 1) {
        for (var i = 0; i < goods.length; i++) {
          for (var j = 0; j < goods.length; j++) {
            if (goods[i].price * 1 < goods[j].price * 1) {
              v = goods[i]
              goods[i] = goods[j]
              goods[j] = v
            }
    ...
    screen: function (e) {
      ...
      if (index == 0) {
        ...
        for (var i = 0; i < goods.length; i++) {
          for (var j = 0; j < goods.length; j++) {
            if (goods[i].sales * 1 > goods[j].sales * 1) {
              v = goods[i]
              goods[i] = goods[j]
              goods[j] = v
            }
