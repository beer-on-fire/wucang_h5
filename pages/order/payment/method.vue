  ...
  <view class="page_box">
    ...
    <view class="content_box">
      <view class="y-f money-box"
        v-if="orderDetail.total_fee">
        <text class="time"
          v-if="isPast">{{ timeText }}</text>
        <view class="money">{{ orderDetail.total_fee }}</view>
      </view>
      <radio-group @change="selPay"
        class="pay-box"
        v-if="payment">
        <label class="x-bc pay-item"
          v-if="payment.includes('wechat')">
          <view class="x-f">
            <image class="pay-img"
              src="http://shopro.7wpp.com/imgs/wx_pay.png"
              mode=""></image>
            <text>微信支付</text>
          </view>
          <radio value="wechat"
            :class="{ checked: payType === 'wechat' }"
            class=" pay-radio orange"
            :checked="payType === 'wechat'"></radio>
        </label>
        <label class="x-bc pay-item"
          v-if="payment.includes('alipay')">
          <view class="x-f">
            <image class="pay-img"
              src="http://shopro.7wpp.com/imgs/ali_pay.png"
              mode=""></image>
            <text>支付宝支付</text>
          </view>
          <radio value="alipay"
        ...
        <label class="x-bc pay-item"
          v-if="payment.includes('iospay') && !isAndroid">
          <view class="x-f">
            <image class="pay-img"
              src="http://shopro.7wpp.com/imgs/apple_pay.png"
              mode=""></image>
            <text>ApplePay</text>
          </view>
          <radio value="iospay"
            :class="{ checked: payType === 'iospay' }"
            class="pay-radio orange"
            :checked="payType === 'iospay'"></radio>
        </label>
        <label class="x-bc pay-item"
          v-if="payment.includes('wallet')">
          <view class="x-f">
            <image class="pay-img"
              src="http://shopro.7wpp.com/imgs/wallet_pay.png"
              mode=""></image>
            <text>余额支付</text>
          </view>
          <radio value="wallet"
            ...
            class="pay-radio orange"
            :checked="payType === 'wallet'"></radio>
        </label>
      </radio-group>
      <view class="x-c">
        <button class="cu-btn pay-btn"
          @tap="confirmPay">确认支付 ￥{{ orderDetail.total_fee }}</button>
      </view>
    </view>
...
<script>
import WuCangPay from '@/common/shopro-pay'
...
export default {
  ...
  methods: {
    ...
    confirmPay () {
      ...
      let pay = new WuCangPay(that.payType, that.orderDetail)
    },
    // 支付信息
    getOrderDetail () {
      let that = this
