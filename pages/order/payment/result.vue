<template>
  <view class="success-page">
    <view class="success-box flex flex-direction align-center"
      ...
      <image class="pay-img"
        :src="pay ? 'http://shopro.7wpp.com/imgs/pay_success.png' : 'http://shopro.7wpp.com/imgs/pay_fail.png'"
        mode=""></image>
      <text class="notice">{{ pay ? '支付成功' : '支付失败' }}</text>
      <text class="pay-money"
        v-if="pay && orderDetail.total_fee">￥{{ orderDetail.total_fee }}</text>
      <view class="btn-box flex justify-between">
        <block v-if="pay && orderDetail.activity_type === 'groupon' && orderDetail.ext_arr.buy_type === 'groupon'">
          ...
          <button class="cu-btn base-btn"
            v-else
            @tap="replace('/pages/activity/groupon/my-groupon')">我的拼团</button>
        </block>

        <button class="cu-btn base-btn"
          v-else
          @tap="routerTo.push('/pages/index/index')">返回首页</button>
        <button class="cu-btn base-btn"
          @tap="onOrder">查看订单</button>
        <button class="again-pay cu-btn"
          v-if="!pay"
          @tap="onPay">重新支付</button>
      </view>
    </view>
    <!-- 登录提示 -->
...
export default {
  ...
  methods: {
    ...
    onPay () {
      let that = this
      let pay = new WuCangPay(that.$Route.query.type, that.orderDetail)
    }
  }
};
</script>
