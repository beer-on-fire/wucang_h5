<template>
  <view class="content b-t">
    ...
    <view class="list b-b"
      ...
      <view class="wrapper">
        <view class="address-box">
          <text v-if="item.is_default"
            class="tag">默认</text>
          <text v-if="!item.is_default"
            class="tag"
            @click="set_default(item.id)">设默认</text>
          <text class="address"
            @click="checkAddress(item.id)">{{item.province}} {{item.city}}{{item.county}}</text>
        </view>
        <view class="u-box"
    ...
    <view class="add-btn">
      <button type="warn"
        style="width: 45%;"
        @click="addAddressA('add')">&emsp;新增地址</button>
      <!-- #ifdef MP-WEIXIN -->
      <button type=""
        style="width: 45%;background-color: #16AB60;color: #fff;"
        @click="get_address">微信地址</button>
      <!-- #endif -->
    </view>
