...
function makeImage(options, callback) {
  ...
  function handleText(options, nextFunc) {
    ...
    for (let i = 0, lineNum = arr.length; i < lineNum; i++) {
      ...
      mainCtx.font = `${options.bold ? `bold ` : ''}${options.size ||
        bodyStyle.fontSize} ${bodyStyle.fontFamily}`
  ...
  function handleImage(options = {}, nextFunc) {
    ...
    // 兼容问题：base64需要特殊处理
    img.src = !~url.indexOf('data:image/')
      ? url
      : URL.createObjectURL(dataURItoBlob(url))
    ...
    const nextFunc = () => {
      ...
      !(len - i)
        ? callback &&
          callback(
            error,
            canvas.toDataURL('image/jpeg', options.compress || 0.8)
          )
        : start()
    }
...
function renderEditor(container, options, callback) {
  ...
  _options.parts = _options.parts.filter(
    item => item.editable && item.type !== 'text'
  )
  ...
  makeImage(_options, (error, data) => {
    ...
    function initEditImage(callback) {
      // 过滤，并添加key值，留下可编辑的图片
      const editImageArr = options.parts.filter((item, key) => {
        item._key = key
    ...
    initEditImage(editImageList => {
      ...
      // 过滤，并添加key值，留下可编辑的文字
      const editTextArr = options.parts.filter((item, key) => {
        item._key = key
      ...
      // 创建视图
      container.innerHTML = `<div class="x-imaging-box">
          <img src="${data}" />
          ${html}
          ${
      ...
      const handleClick = e => {
        ...
        if (e.target.className === 'x-make-image') {
          const textDom = document.getElementsByClassName(
